...
int main(int argc, char *argv[])
{
	...
	__Tcall_assert(gfd, evl_new_mutex(&lock_1, "%s", name));

	name = get_unique_name(EVL_MONITOR_DEV, 1);
	__Tcall_assert(gfd, evl_new_mutex(&lock_2, "%s", name));

	name = get_unique_name(EVL_MONITOR_DEV, 2);
	__Tcall_assert(sfd, evl_new_flags(&start, "%s", name));

	name = get_unique_name(EVL_MONITOR_DEV, 3);
	__Tcall_assert(sfd, evl_new_sem(&stop, "%s", name));

	name = get_unique_name(EVL_MONITOR_DEV, 4);
	__Tcall_assert(sfd, evl_new_sem(&init, "%s", name));
